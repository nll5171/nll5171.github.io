...
window.onload = () => {
  const maxWidth =
    (Math.floor(document.querySelector('#puzzle-display').clientWidth / 16) - 12) * 16
  const maxHeight =
    (Math.floor(document.querySelector('#puzzle-display').clientHeight / 16) - 8) * 16
  ...
  const imageNames = ['goomba', 'kirby', 'link', 'mario', 'waddle-dee']
  ...
  function displayPuzzle() {
    ...
    for (let a = 0; a < puzzleColumns.length; a++) {
      ...
      } else currentElement.style.borderRight = '1px solid black'
    ...
    for (let a = 0; a < puzzleRows.length; a++) {
      ...
      } else currentElement.style.borderBottom = '1px solid black'
  ...
  function removeBlockInteraction() {
    for (let a = 0; a < selectedBlocks.length; a++) {
      for (let b = 0; b < selectedBlocks[a].length; b++) selectedBlocks[a][b].interactive = false
    }
  }
  ...
  function displayCompletePuzzle() {
    let currentBlock =
      selectedBlocks[Math.floor(blockCounter / selectedBlocks.length)][
        blockCounter % selectedBlocks[0].length
      ]
