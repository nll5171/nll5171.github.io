      ...
      Promise.all([monochromePromise, colorPromise]).then(() => {
        ...
        for (let a = 0; a < this.colorImage.width; a++) {
          ...
          for (let b = 0; b < this.colorImage.height; b++) {
            ...
            for (let c = 0; c < 3; c++)
              completeColor += cRGBA[(this.colorImage.width * a + b) * 4 + c]
                .toString(16)
                .padStart(2, '0')

            let block = new PixelBlock(
              filled,
              completeColor,
              blockWidth * b,
              blockHeight * a,
        ...
        for (let a = 0; a < this.blocks.length; a++) {
          ...
          for (let b = 0; b < currentRow.length; b++) {
            if (currentRow[b].filled) consecutiveBlocks++
          ...
          if (consecutiveBlocks > 0 || rowHints.length == 0) rowHints.push(consecutiveBlocks)
        ...
        for (let a = 0; a < this.blocks[0].length; a++) {
          ...
          for (let b = 0; b < this.blocks.length; b++) {
            if (this.blocks[b][a].filled) consecutiveBlocks++
            else if (consecutiveBlocks > 0) {
          ...
          if (consecutiveBlocks > 0 || columnHints.length == 0) columnHints.push(consecutiveBlocks)
